//seven: make these functions global.
var formPlaceHoldersInitializer;
var formPlaceholderTextRemover;
...
(function($){
	...
	$(document).ready( function(){
		...
		var $search_form = $et_search_icon.siblings( '.et-search-form' );
		$search_form.bind('submit', function(event){
			if ($(this).find('.et-search-field').attr('value').length == 0)
			{
		...
		if ( $et_pb_video_section.length ){
		    var $mediaElementPlayer = $et_pb_video_section.find( 'video' ).mediaelementplayer;
        ...
        formPlaceHoldersInitializer = et_pb_form_placeholders_init;
        ...
        formPlaceholderTextRemover = et_pb_remove_placeholder_text;
		...
		$( window ).resize( function(){
			...
			if ( $( '.et_pb_blog_grid' ).length && $( '.et_pb_blog_grid' ).masonry )
				$( '.et_pb_blog_grid' ).masonry('reload');

			if ( et_is_fixed_nav && containerWidthChanged ) {
				setTimeout( function() {
                    //引起一个bug：chrome下放大缩小之后导致padding-top不对
